...
public class MetricCollector implements MobsimBeforeSimStepListener {
    ...
    public void notifyMobsimBeforeSimStep(MobsimBeforeSimStepEvent e) {
      //PUT CODE HERE
        ...
        if((timeStep%600)==0){
            logger.info("IM INSIDE THIS BITCH");
            for(Id<Vehicle> vehicleId : vehMap.keySet()) {
                ...
                File linkFile = new File("/Users/ankitanand/Desktop/"+linkFileName);
                PrintWriter out = null;
                if ( linkFile.exists() && !linkFile.isDirectory() ) {
                    try {
                        Log.info("file appending");
                        out = new PrintWriter(new FileOutputStream(new File("/Users/ankitanand/Desktop/"+linkFileName), true));
                    ...
                    try {
                        ...
                        out = new PrintWriter("/Users/ankitanand/Desktop/"+linkFileName);
                        Log.info("write complete");
                    } catch (FileNotFoundException ex) {
                        ex.printStackTrace();
                    }
                ...
                File scoreFile = new File("/Users/ankitanand/Desktop/"+scoreFileName);
                if ( scoreFile.exists() && !scoreFile.isDirectory() ) {
                    try {
                        Log.info("file appending");
                        out = new PrintWriter(new FileOutputStream(new File("/Users/ankitanand/Desktop/"+scoreFileName), true));
                        Log.info("write complete");
                    } catch (FileNotFoundException ex) {
                        ex.printStackTrace();
                    }
                    ...
                    try {
                        Log.info("file creating");
                        out = new PrintWriter("/Users/ankitanand/Desktop/"+scoreFileName);
                        Log.info("write complete");
                    } catch (FileNotFoundException ex) {
                        ex.printStackTrace();
                    }
